            ...
            @if($items->count())
                @foreach($items as $item)
                    <tr>
                        ...
                        <td class="text-center">
                            <a href="{{url('leaders/'.$item->id)}}" class="text-success" title="View">
                                <i class="nav-icon i-Eye1 font-weight-bold"></i>
                            </a>

                            <a href="{{url('leaders/'.$item->id.'/edit')}}" class="text-black-50" title="Edit">
                                <i class="nav-icon i-Pen-2 font-weight-bold"></i>
                            </a>
                            @if(!$item->is_approved)
                                <form id="approve-form-{{$item->id}}" method="post" class="approve-form"
                                      action="{{route('leader-approve',$item->id)}}">
                                    {{csrf_field()}}
                            ...
                            <form id="delete-form-{{$item->id}}" method="post" class="delete-form"
                                  action="{{route('leader-delete',$item->id)}}">
                                {{csrf_field()}}
