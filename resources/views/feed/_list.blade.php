            ...
            @if($items->count())
                @foreach($items as $item)
                    <tr>
                        ...
                        <td class="text-center">{{ $item->category_id ? $item->category->name : ''}}</td>
                        <td class="text-center">{{ $item['created_at'] }}</td>
                        <td class="text-center">
                            <a href="{{route('feed',$item->id)}}" class="text-success">
                                <i class="nav-icon i-Eye1 font-weight-bold"></i>
                            </a>
{{--                            <a href="{{route('admin-post-edit',$item->id)}}" class="text-success">--}}
{{--                                <i class="nav-icon i-Pen-2 font-weight-bold"></i>--}}
{{--                            </a>--}}
{{--                            <form id="delete-form-{{$item->id}}" method="post" class="delete-form"--}}
{{--                                  action="{{route('feed-delete',$item->id)}}">--}}
{{--                                {{csrf_field()}}--}}
